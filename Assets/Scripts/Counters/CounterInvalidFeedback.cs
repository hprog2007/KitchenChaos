...
public class CounterInvalidFeedback : MonoBehaviour
{
    [Header("Shake")]
    [SerializeField] private Transform shakeTarget;          // default: this.transform
    ...
    void Awake()
    {
        ...
        foreach (var r in flashRenderers)
            if (r && !_orig.ContainsKey(r)) _orig[r] = GetColor(r);
    }

    void OnEnable() { counter.OnInvalidInteraction += HandleInvalid; }
    void OnDisable()
    {
        counter.OnInvalidInteraction -= HandleInvalid;
        if (shakeCo != null) StopCoroutine(shakeCo);
        if (flashCo != null) StopCoroutine(flashCo);
        RestoreColors();
    }

    private void HandleInvalid(InvalidInteractionReason reason, KitchenObjectSO so)
    {
    ...
    {
        foreach (var r in flashRenderers) SetColor(r, flashColor);
